define(
    ...
    function (jquery, async, qna, qnautils, qnazipmodel, qnastart, restcalls, specelement, URI, constants, reportsettings, moment, xmlcompare, exports) {
        ...
        function isImportingFromRemote(config) {
            return config.ImportOption === constants.MOJO_IMPORT_OPTION;
        }
        ...
        function updatingTopics(config) {
            return config[constants.CREATE_OR_REUSE_TOPICS] !== constants.CREATE_TOPICS &&  config[constants.CREATE_OR_OVERWRITE_CONFIG_KEY] === constants.OVERWRITE_SPEC;
        }

        function reusingTopics(config) {
            return config[constants.CREATE_OR_REUSE_TOPICS] !== constants.CREATE_TOPICS &&  config[constants.CREATE_OR_OVERWRITE_CONFIG_KEY] === constants.CREATE_SPEC;
        }

        function creatingTopics(config) {
            return config[constants.CREATE_OR_REUSE_TOPICS] === constants.CREATE_TOPICS;
        }
        ...
        var processZipFile = new qna.QNAStep()
            ...
            .setEnterStep(function (resultCallback, errorCallback, result, config) {
                ...
                function uploadImages (xmlDoc, contentSpec, topics, topicGraph, callback) {
                    ...
                    if (isImportingFromRemote(config)) {
                ...
                new qna.QNAVariables()
                    .setVariables(function (resultCallback, errorCallback, result, config) {
                        ...
                        variables.push(new qna.QNAVariable()
                            ...
                            .setValue(function (resultCallback, errorCallback, result, config) {
                                resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#ContentSpecFilteredResultsAndContentSpecView;query;contentSpecIds=" + config.ContentSpecID + "'>" + config.ContentSpecID + "</a> (Click to open in PressGang)");
                            }));
                        variables.push(new qna.QNAVariable()
                            .setType(qna.InputEnum.PLAIN_TEXT)
                        ...
                        variables.push(new qna.QNAVariable()
                            ...
                            .setValue(function (resultCallback, errorCallback, result, config) {
                                if (config.OutgoingUrls.length === 0) {
                                    ...
                                    resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#SearchResultsAndTopicView;query;topicIds=" + config.OutgoingUrls + "'</a>Go to topics with outgoing urls</a>");
                                }
                            }));
                        variables.push(new qna.QNAVariable()
                            .setType(qna.InputEnum.HTML)
                            ...
                            .setValue(function (resultCallback, errorCallback, result, config) {
                                if (config.NewTopics.length === 0) {
                                    ...
                                    resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#SearchResultsAndTopicView;query;topicIds=" + convertArrayToCommaSeparatedString(config.NewTopics) + "'</a>Go to new topics that were created as part of this import</a>");
                                }
                            }));
                        variables.push(new qna.QNAVariable()
                            .setType(qna.InputEnum.HTML)
                            ...
                            .setValue(function (resultCallback, errorCallback, result, config) {
                                if (config.UpdatedTopics.length === 0) {
                                    ...
                                    resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#SearchResultsAndTopicView;query;topicIds=" + convertArrayToCommaSeparatedString(config.UpdatedTopics) + "'</a>Go to existing topics that were updated as part of this import</a>");
                                }
                            }));
                        variables.push(new qna.QNAVariable()
                            .setType(qna.InputEnum.HTML)
                            ...
                            .setValue(function (resultCallback, errorCallback, result, config) {
                                if (config.ReusedTopics.length === 0) {
                                    ...
                                    resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#SearchResultsAndTopicView;query;topicIds=" + convertArrayToCommaSeparatedString(config.ReusedTopics) + "'</a>Go to existing topics that were reused as part of this import</a>");
                                }
                            }));
                        if (updatingTopics(config)) {
                            variables.push(new qna.QNAVariable()
                                ...
                                .setValue(function (resultCallback, errorCallback, result, config) {
                                    if (config.RemovedTopics.length === 0) {
                                        ...
                                        resultCallback("<a href='http://" + config[constants.PressGangHost] + ":8080/pressgang-ccms-ui/#SearchResultsAndTopicView;query;topicIds=" + convertArrayToCommaSeparatedString(config.RemovedTopics) + "'</a>Go to existing topics that were discarded as part of this import</a>");
                                    }
                                }));
                        }
                        resultCallback(variables);
